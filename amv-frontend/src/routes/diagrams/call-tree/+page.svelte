<script lang="ts">
  ...
  let { signaturePattern, callTreeRequired, calledTreeRequired } = $state(data.criteria as CallTreeCriteriaModel);
  ...
  $effect(() => {
    if (callTrees && callTrees.length > 0) {
        ...
        {#if method.srcUrl}
          ...
          <span
            on:click|preventDefault={() => {
              criteria.methodCriteria.text = method.qualifiedSignature;
            }}
            on:keydown={(e) => {
